...
import { findVolunteerByLogin, setCurrentUser } from "@/lib/store";
...
export default function VolunteerLoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const user = findVolunteerByLogin(identifier, password);
    if (!user) {
    ...
    if (user.status === 'pending') {
      toast.warning("Account Pending", {
        description: "Your account is awaiting admin approval. We'll notify you soon.",
      });
    ...
    toast.success("Welcome back, " + user.fullName + "!", {
      description: "You've been successfully signed in as a volunteer.",
    });
        ...
        <div className="rounded-2xl bg-card p-8 shadow-card border border-border/50">
          <div className="space-y-6">
            ...
            <button 
              ...
              className="w-full flex items-center justify-center gap-3 p-4 rounded-xl border-2 border-border bg-background text-foreground font-medium transition-all hover:border-primary/30 hover:shadow-soft"
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <motion.button 
                ...
                whileTap={{ scale: 0.98 }} 
                type="submit" 
                className="w-full py-4 rounded-xl bg-primary text-primary-foreground font-display font-bold text-base flex items-center justify-center gap-2 shadow-md"
                aria-label="Sign in"
              >
